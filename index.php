	...
	<header>
		<div class="container position-relative">
			...
			<div class="selector-idioma">
				<ul style="top: 10px;">
					<li>
						<a <?php if($lang=='es'){ ?>class="selected"<?php } ?>href="javascript:void(0);" data-lang="es" onclick="gtag('event', 'Botón', {  'event_category': 'Idioma Español','event_label': 'Home' });">esp</a>
					</li>
					<li class="divisor">|</li>
					<li>
						<a <?php if($lang=='en'){ ?>class="selected"<?php } ?> href="javascript:void(0);" data-lang="en" onclick="gtag('event', 'Botón', {  'event_category': 'Idioma Español','event_label': 'Home' });">eng</a>
					</li>
				</ul>
			</div>
			<div class="row">
				<span class="menu-icon" onclick="openNav(); gtag('event', 'Botón', {  'event_category': 'Menú','event_label': 'Home' });">
				...
				<div class="col-m-12 text-center">
					...
					<form id="formEnviarCalorias" action="ajax/save-calories.php" class="form-inline" method="post">
						<div class="form-group">
							<input type="number" class="form-control" name="numCalorias" id="numCalorias" placeholder="" required="" min="1" data-msg="<?php if($lang=='en'){ ?>Log your burned calories here<?php }else{ ?>Ingresa aquí tus calorías quemadas<?php } ?>">
						</div>
						<div id="flechita"></div>
<!-- 						<img src="assets/img/flecha.png" alt=""> -->
						...
						<!--logueado -->
						<button type="submit" class="btn btn-default btn-enviar-calorias<?php if($lang=='en'){ ?>-eng<?php } ?>" onclick="gtag('event', 'Botón', {  'event_category': 'Enviar Calorías','event_label': 'Home' });"></button>
					...
					<p class="txt2"><?php if($lang=='en'){ ?>Don’t know? Calculate <?php }else{ ?> ¿NO SABES? CALCULAR <?php } ?><a href="#" data-toggle="modal" data-target="#modal-calcula" onclick="gtag('event', 'Botón', {  'event_category': 'Calcular','event_label': 'Home' });"><?php if($lang=='en'){ ?>here<?php }else{ ?>AQUÍ<?php } ?></a> </p>
				</div>
			</div>
		</div>
		...
		<div class="container">
			...
			<div class="box-contador">
				<div class="range-container">
					<p class="txt-llevamos"><?php if($lang=='en'){ ?>WE’VE REACHED<?php }else{ ?>llevamos<?php } ?> <?php echo $datoLlevamos;?> <?php if($lang=='en'){ ?>MILLION calories<?php }else{ ?>Millones de calorías<?php } ?> <br>
						<?php if($lang=='en'){ ?>(500 calories = 1 meal)<?php }else{ ?>(500 calorías = 1 comida)<?php } ?>
						</p>
					<div class="llevamos" style="width: <?php echo $datoPorcentaje."%";?>">
						<div class="t-naranjo"></div>
					</div>
					<div class="total">
						<p class="txt-total"><?php echo $datoMeta;?> <?php if($lang=='en'){ ?>Million calories<?php }else{ ?>Millones de calorías<?php } ?></p>
	...
	<section class="compra-polera section">
		<div class="container">
			<div class="row">
				<div class="col-sm-12">
					<a href="https://www.k1.cl/modules/burntogive/" target="_blank" onclick="gtag('event', 'Botón', {  'event_category': 'Banner Poleras','event_label': 'Home' });">
